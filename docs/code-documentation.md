...
本约定面向 UnicodeArtJs 的贡献者和二次开发者。它用于保持 Core、CLI、Web 和 VS Code Extension 的公共契约可读、可追溯，并避免把内部计划、会话记录或实现细枝末节误写成对外承诺。
...
- 文档只描述公开 API、配置、格式、兼容性与安全边界。不得扫描或链接私有规划资料、会话日志、内部审计或本地生成目录。
...
6. 运行 `npm run docs:contract:check`；修改 Core 公开 TypeScript 契约时，再运行 `npm run docs:tsdoc:core:check`；修改 VS Code Extension 宿主边界时，再运行 `npm run docs:tsdoc:vscode:check`；涉及 CLI 或 Web 文档试点时，再运行对应的 `docs:*:check`。
