...
- JavaScript 以 HIA JSDoc 为当前生成入口。TypeScript 的正式生成入口将在 HIA Documentation Sys 的 TSDoc 能力发布并通过仓库 probe 后接入；在此之前，TypeScript 注释仍按本约定编写，但不宣称已生成公开 API 页面。
...
6. 运行 `npm run docs:contract:check`；涉及 CLI 文档试点时，再运行 `npm run docs:cli:check`。
