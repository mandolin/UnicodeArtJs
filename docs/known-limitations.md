...
如果你正在升级版本或把 Core 接入新宿主，先看 [迁移指南](migration-guide.md)。如果只是想快速判断一个现象属于哪类限制，可以先看下表。

## 按现象定位

| 现象 | 优先阅读 |
| --- | --- |
| 字体选项变了但显示效果没变 | [字体行为与浏览器回退](font-behavior.md) |
| 浏览器与 Node 输出不完全一样 | 本页“浏览器入口”和“Node 图片与文字后端” |
| 图片格式不支持或结果为空 | [可选输入格式与 Adapter 策略](optional-input-adapters.md) |
| 旧配置字段和新 UI 字段对不上 | [配置模型 vNext](config-model-vnext.md) |
| 不确定 experimental 能否用于长期项目 | [实验能力稳定性矩阵](experimental-stability.md) |
