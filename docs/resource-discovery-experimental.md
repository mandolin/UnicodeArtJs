...
该校验器只读取仓库内的静态文件：它会确认清单与 `gallery/index.json` 一致、资源路径没有越出 `artworks/`、文件大小和 sha256 与实际内容一致，并对 UAF / 语义文档做基本结构检查。它不联网、不下载、不安装，也不执行资源内容。
...
2. 读取资源文件后重新计算 hash。
3. 调用 Core 校验 UAF、语义文档或 UAEM。
4. 在用户确认后再导入到当前工作区。
5. 缓存时记录来源、hash、Core 版本和导入时间。
6. 发现撤回或校验失败时，不替换用户当前内容。
