...
7. 新资源种类必须先进入公开格式设计，不能直接塞进 UAEM v1 的未知字段。

官方随仓示例 `packages/extension-line-banner` 是当前基线：它包含一份原创 UAF 字体和一份语义文档模板，用于证明清单、路径、许可证和资源解析可以在不执行第三方代码的情况下完成。
...
该命令会构建 Core，检查公开 SDK 文档、官方示例包、Core 清单解析、CLI 侧载预检、Web 只读清单测试入口以及 release gate 文档。它不会安装扩展，也不会访问网络。
