...
宿主展示资源发现信息后，仍需要单独的导入确认。确认界面应展示资源标题、类型、许可证、来源、size、sha256、
信任状态、撤回状态和缓存目标；导入前重新计算 hash，并确认资源没有被撤回。用户取消、hash 不一致、签名状态
不满足宿主策略或 Core 校验失败时，不替换当前项目、模板、预览或编辑器内容。
