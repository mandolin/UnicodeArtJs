...
| CLI | 已发布的 `unicode-art-cli@1.0.2` 声明 `unicode-art-js@^1.2.0` | CLI 与 Core 通过共享 fixture 保持输出契约。发布新版 CLI 前应把依赖范围调整为当前 Core 发布线并执行发布门禁。 |
...
- 项目文件 `*.uaproj` 采用版本化 JSON。新版本读取旧项目时必须保留未知字段或给出明确错误；不要静默覆盖用户原始项目。
