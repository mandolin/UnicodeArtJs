...
同目录还包含 `resource-manifest.json`，它为每个公开资源记录 size 与 sha256。在线工具的“资源发现”实验页会读取这份同源清单并在浏览器端重新计算 hash，用于展示随站资源是否仍与清单一致。维护者修改画廊资源后，应同时运行 `npm run gallery:check`、`npm run resource-discovery:check` 和 `npm run web-resource-discovery:check`，也可用 `unicode-art resource validate <manifest>` 做 CLI 只读预检，确保索引、资源文件和 hash 清单保持一致。
