...
同目录还包含 `resource-manifest.json`，它为每个公开资源记录 size 与 sha256。在线工具的“资源发现”实验页会读取这份同源清单并在浏览器端重新计算 hash，用于展示随站资源是否仍与清单一致；通过维护者签名和撤回检查的资源，可以在用户确认后导入 source-first 编辑器。画廊还发布 `resource-lock.json`、`resource-revocations.json` 和 `resource-signature.json`，用于 experimental hash lock、撤回列表和当前 `maintainer-signed` 签名 envelope。维护者修改画廊资源后，应同时运行 `npm run gallery:check`、`npm run resource-discovery:check`、`npm run resource-trust:check` 和 `npm run web-resource-discovery:check`，也可用 `unicode-art resource validate <manifest>` 做 CLI 只读预检，确保索引、资源文件和 hash 清单保持一致。
