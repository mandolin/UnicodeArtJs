...
| 能力 | 格式 | 当前定位 | 主要入口 |
| --- | --- | --- | --- |
| Unicode Art Font | `unicode-art-font@1` | Beta 候选艺术字字体格式 | [UAF 与语义布局 Beta 契约](semantic-uaf-beta.md) |
| Semantic Document | `semantic-document@1` | Beta 候选语义布局 JSON AST | [UAF 与语义布局 Beta 契约](semantic-uaf-beta.md) |
...
2. 如果要制作艺术字字体，阅读 [UAF 与语义布局 Beta 契约](semantic-uaf-beta.md)，并从 `packages/extension-line-banner/assets/line-font.uafont.json` 复制最小结构。
3. 如果要制作布局模板，优先写 `semantic-document@1` JSON，而不是把受限 DSL 当作长期保存格式。
4. 如果要分发本地资源包，使用 [声明式扩展作者指南](extension-authoring.md) 编写 `unicode-art-extension.json`。
5. 如果要提交默认画廊，先阅读 [静态画廊投稿指南](gallery-submission.md)，确认作品来源、许可证、双语说明和审核材料。
