...
| Static Resource Discovery | experimental 可采用静态资源发现 | 展示资源 ID、来源、许可证、hash 和作者 / 作品入口；当前已提供同源 `resource-manifest.json` 校验、hash lock / 撤回 / `unsigned-draft` envelope、Web 只读实验页与 CLI 只读 inspect / validate | [实验性静态资源发现](resource-discovery-experimental.md) |
