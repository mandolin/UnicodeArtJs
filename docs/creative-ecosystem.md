...
| Static Resource Discovery | experimental 可采用静态资源发现 | 展示资源 ID、来源、许可证、hash 和作者 / 作品入口；当前已提供同源 `resource-manifest.json` 校验、hash lock / 撤回 / `maintainer-signed` envelope、Web 确认导入实验页与 CLI 只读 inspect / validate | [实验性静态资源发现](resource-discovery-experimental.md) |
...
当前创作格式仍处于 experimental / beta 候选区间。静态资源发现已经具备高级用户和宿主开发者可试用的最小链路，但它仍不是动态市场，也不会自动安装资源。进入稳定候选前，需要至少满足：
