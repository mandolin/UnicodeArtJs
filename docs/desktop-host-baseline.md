...
桌面宿主可以在后续版本展示官方静态资源索引或用户选择的本地资源清单，但发现列表不等于导入。导入到项目、
模板、资源库或应用私有缓存前，应通过原生对话框或受控页面展示资源标题、类型、许可证、来源、size、sha256、
信任状态、撤回状态、目标项目和缓存目标。linked 图片、外部资源路径和远程下载结果都需要用户重新授权。

如果 hash 不一致、资源被撤回、签名状态不满足宿主策略、Core 校验失败或用户取消，桌面宿主不得替换当前
`*.uaproj`、预览、模板或缓存索引。缓存记录应包含来源、hash、Core 版本、导入时间和用户确认状态，方便后续
禁用、删除或回退。
