...
#### resource 命令（实验性）
    ...
    unicode-art resource validate <manifest> [--json] [--lang zh-CN|en-US]
    unicode-art resource inspect <manifest> [--json] [--lang zh-CN|en-US]

- manifest 是本地 `resource-manifest.json` 文件，不能使用 stdin。
- 当前支持 `unicode-art-gallery-resource-manifest@1`，用于校验静态画廊资源的 ID、类型、路径、
  size、sha256、许可证和审核日期。
- validate 会读取 manifest 同目录下声明的 gallery index 与 `resourceRoot` 内资源，并调用 Core
  检查 UAF 字体或语义文档格式。
- inspect 输出相同摘要；`--json` 可用于 CI、发布脚本或后续宿主工具。
- 两个命令都不会联网、下载、安装、导入或执行资源内容。读取索引和资源前会复核 realpath
  没有越出 manifest 所在目录和资源根目录。
- 后续若增加资源导入命令，也必须要求显式输出路径或目标配置。当前 `resource` 子命令只做验证与摘要，
  不写入全局模板、配置文件或缓存。

可用仓库内置清单试运行：

```bash
unicode-art resource validate packages/web/public/gallery/resource-manifest.json --lang zh-CN
